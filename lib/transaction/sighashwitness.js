 ...
 * @param {Buffer} litoshisBuffer
 */
var sighash = function sighash(transaction, sighashType, inputNumber, scriptCode, litoshisBuffer) {
 ...
 * @return {Signature}
 */
function sign(transaction, privateKey, sighashType, inputIndex, scriptCode, litoshisBuffer) {
  var hashbuf = sighash(transaction, sighashType, inputIndex, scriptCode, litoshisBuffer);
 ...
 * @return {boolean}
 */
function verify(transaction, signature, publicKey, inputIndex, scriptCode, litoshisBuffer) {
  $.checkArgument(!lodash.isUndefined(transaction));
  $.checkArgument(!lodash.isUndefined(signature) && !lodash.isUndefined(signature.nhashtype));
  var hashbuf = sighash(transaction, signature.nhashtype, inputIndex, scriptCode, litoshisBuffer);
