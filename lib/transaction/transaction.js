...
// max amount of litoshis in circulation
...
// Value used for fee estimation (litoshis per kilobyte)
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more litecoins than the sum of the input amounts
 * @return {string}
 */
...
Transaction.prototype.checkedSerialize = function(opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'https://openwalletstack.com/api/lib/transaction#serialization-checks';
    throw serializationError;
  }
  return this.uncheckedSerialize();
};

Transaction.prototype.invalidLitoshis = function() {
  var invalid = false;
  for (var i = 0; i < this.outputs.length; i++) {
    if (this.outputs[i].invalidLitoshis()) {
      invalid = true;
    }
  }
  return invalid;
...
Transaction.prototype.getSerializationError = function(opts) {
  ...
  if (this.invalidLitoshis()) {
    return new errors.Transaction.InvalidLitoshis();
  }
...
Transaction.prototype._hasDustOutputs = function(opts) {
  ...
  for (index in this.outputs) {
    ...
    if (output.litoshis < Transaction.DUST_AMOUNT && !output.script.isDataOut()) {
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  $.checkArgument(lodash.isObject(arg) || arg instanceof Transaction);
  var self = this;
  var transaction;
  ...
  lodash.each(transaction.inputs, function(input) {
    ...
    if (script.isPublicKeyHashOut()) {
      ...
      txin = new Input.MultiSigScriptHash(
        input, input.publicKeys, input.threshold, input.signatures, input.nestedWitness
      );
  ...
  lodash.each(transaction.outputs, function(output) {
...
Transaction.prototype.lockUntilBlockHeight = function(height) {
  $.checkArgument(lodash.isNumber(height));
 ...
 * and a slightly fancier format recognized by ltcLib:
 *
 * ```
 * {
 ...
 * transaction.from({'txId': '0000...', outputIndex: 0, litoshis: 1000, script: 'OP_DUP ...'});
 *
 * // From a multisig P2SH output
 * transaction.from({'txId': '0000...', inputIndex: 0, litoshis: 1000, script: '... OP_HASH'},
 *                  ['03000...', '02000...'], 2);
 * ```
 *
...
Transaction.prototype.from = function(utxo, pubkeys, threshold, nestedWitness) {
  if (lodash.isArray(utxo)) {
    ...
    lodash.each(utxo, function(utxo) {
      self.from(utxo, pubkeys, threshold, nestedWitness);
    });
    return this;
  }
 ...
 * set, two additional parameters, `outputScript` and `litoshis` can be provided.
 *
 * @param {Input} input
 * @param {String|Script} outputScript
 * @param {number} litoshis
 * @return Transaction this, for chaining
 */
Transaction.prototype.addInput = function(input, outputScript, litoshis) {
  $.checkArgumentType(input, Input, 'input');
  if (!input.output && (lodash.isUndefined(outputScript) || lodash.isUndefined(litoshis))) {
    throw new errors.Transaction.NeedMoreInfo('Need information about the UTXO script and litoshis');
  }
  if (!input.output && outputScript && !lodash.isUndefined(litoshis)) {
    outputScript = outputScript instanceof Script ? outputScript : new Script(outputScript);
    $.checkArgumentType(litoshis, 'number', 'litoshis');
 ...
 * be reset).
 *
 * @param {number} amount litoshis to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * be reset).
 *
 * @param {number} amount litoshis per KB to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * @param {number} amount in litoshis
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount) {
  if (lodash.isArray(address)) {
    var self = this;
    lodash.each(address, function(to) {
      self.to(to.address, to.litoshis);
    });
    return this;
  }
...
/**
 * Calculates or gets the total output amount in litoshis
 *
...
Transaction.prototype._getOutputAmount = function() {
  if (lodash.isUndefined(this._outputAmount)) {
    ...
    lodash.each(this.outputs, function(output) {
      self._outputAmount += output.litoshis;
    });
  }
...
/**
 * Calculates or gets the total input amount in litoshis
 *
...
Transaction.prototype._getInputAmount = function() {
  if (lodash.isUndefined(this._inputAmount)) {
    ...
    lodash.each(this.inputs, function(input) {
      ...
      self._inputAmount += input.output.litoshis;
    });
  }
 ...
 *
 * @return {Number} fee of this transaction in litoshis
 */
...
Transaction._estimateFee = function(size, amountAvailable, feePerKb) {
  var fee = Math.round((size / 1000) * (feePerKb || Transaction.FEE_PER_KB));
  if (amountAvailable > fee) {
    size += Transaction.CHANGE_OUTPUT_MAX_SIZE;
  }
  return Math.round((size / 1000) * (feePerKb || Transaction.FEE_PER_KB));
};
...
Transaction.prototype.sort = function() {
  ...
  this.sortOutputs(function(outputs) {
    ...
    copy.sort(function(first, second) {
      return first.litoshis - second.litoshis
 ...
 * version of the Fisher-Yates shuffle, provided by lodash's lodash.shuffle().
 *
 * @return {Transaction} this
 */
...
Transaction.prototype.sign = function(privateKey, sigtype) {
  ...
  lodash.each(this.getSignatures(privateKey, sigtype), function(signature) {
...
Transaction.prototype.getSignatures = function(privKey, sigtype) {
  ...
  lodash.each(this.inputs, function forEachInput(input, index) {
    lodash.each(input.getSignatures(transaction, privKey, index, sigtype, hashData), function(signature) {
 ...
 */
Transaction.prototype.verifySignature = function(sig, pubkey, nin, subscript, sigversion, litoshis) {
  ...
  if (sigversion === 1) {
    ...
    var litoshisBuffer;
    if (litoshis) {
      $.checkState(JSUtil.isNaturalNumber(litoshis));
      litoshisBuffer = new BufferWriter().writeUInt64LEBN(new BN(litoshis)).toBuffer();
    } else {
      litoshisBuffer = this.inputs[nin].getLitoshisBuffer();
    }
...
Transaction.prototype.verify = function() {
  ...
  for (var i = 0; i < this.outputs.length; i++) {
    ...
    if (txout.invalidLitoshis()) {
      return 'transaction txout ' + i + ' litoshis is invalid';
    }
    if (txout._litoshisBN.gt(new BN(Transaction.MAX_MONEY, 10))) {
      return 'transaction txout ' + i + ' greater than MAX_MONEY';
    }
    valueoutbn = valueoutbn.add(txout._litoshisBN);
