...
/**
 * A litecoin transaction script. Each transaction's inputs and outputs
...
Script.prototype.set = function(obj) {
  $.checkArgument(lodash.isObject(obj));
  $.checkArgument(lodash.isArray(obj.chunks));
...
Script.buildMultisigOut = function(publicKeys, threshold, opts) {
  ...
  publicKeys = lodash.map(publicKeys, PublicKey);
...
Script.buildMultisigIn = function(pubkeys, threshold, signatures, opts) {
  $.checkArgument(lodash.isArray(pubkeys));
  $.checkArgument(lodash.isNumber(threshold));
  $.checkArgument(lodash.isArray(signatures));
  opts = opts || {};
  var s = new Script();
  s.add(Opcode.OP_0);
...
Script.buildPublicKeyHashOut = function(to) {
  $.checkArgument(!lodash.isUndefined(to));
  $.checkArgument(to instanceof PublicKey || to instanceof Address || lodash.isString(to));
  if (to instanceof PublicKey) {
    to = Address.fromPublicKey(to);
...
Script.buildDataOut = function(data, encoding) {
  $.checkArgument(lodash.isUndefined(data) || lodash.isString(data) || BufferUtil.isBuffer(data));
...
Script.buildPublicKeyIn = function(signature, sigtype) {
  ...
  $.checkArgument(lodash.isUndefined(sigtype) || lodash.isNumber(sigtype));
