...
/**
 * Litecoin transactions contain scripts. Each input has a script called the
...
Interpreter.prototype.verifyWitnessProgram = function(version, program, witness, litoshis, flags) {
 ...
 * @param {number} litoshis - number of litoshis created by this output
 *
 * Translated from bitcoind's VerifyScript
 */
Interpreter.prototype.verify = function(scriptSig, scriptPubkey, tx, nin, flags, witness, litoshis) {
  ...
  if (lodash.isUndefined(litoshis)) {
    litoshis = 0;
  }
  ...
  if ((flags & Interpreter.SCRIPT_VERIFY_WITNESS)) {
    ...
    if (scriptPubkey.isWitnessProgram(witnessValues)) {
      ...
      if (!this.verifyWitnessProgram(witnessValues.version, witnessValues.program, witness, litoshis, flags)) {
        return false;
      }
    }
  }
  ...
  if ((flags & Interpreter.SCRIPT_VERIFY_P2SH) && scriptPubkey.isScriptHashOut()) {
    ...
    if ((flags & Interpreter.SCRIPT_VERIFY_WITNESS)) {
      ...
      if (redeemScript.isWitnessProgram(p2shWitnessValues)) {
        ...
        if (!this.verifyWitnessProgram(p2shWitnessValues.version, p2shWitnessValues.program, witness, litoshis, flags)) {
          return false;
        }
...
Interpreter.prototype.set = function(obj) {
  ...
  this.litoshis = typeof obj.litoshis !== 'undefined' ? obj.litoshis : this.litoshis;
      ...
      case Opcode.OP_CHECKSIGVERIFY:
        {
          ...
          try {
            ...
            fSuccess = this.tx.verifySignature(sig, pubkey, this.nin, subscript, this.sigversion, this.litoshis);
          } catch (e) {
      ...
      case Opcode.OP_CHECKMULTISIGVERIFY:
        {
          ...
          while (fSuccess && nSigsCount > 0) {
            ...
            try {
              sig = Signature.fromTxFormat(bufSig);
              pubkey = PublicKey.fromBuffer(bufPubkey, false);
              fOk = this.tx.verifySignature(sig, pubkey, this.nin, subscript, this.sigversion, this.litoshis);
            } catch (e) {
              //invalid sig or pubkey
              fOk = false;
            }
