...
describe('Output', function() {
  ...
  it('can be assigned a litoshi amount in big number', function() {
    var newOutput = new Output({
      litoshis: new BN(100),
      script: Script.empty()
    });
    newOutput.litoshis.should.equal(100);
  });

  it('can be assigned a litoshi amount with a string', function() {
    var newOutput = new Output({
      litoshis: '100',
      script: Script.empty()
    });
  ...
  describe('will error if output is not a positive integer', function() {
    it('-100', function() {
      ...
      }.should.throw('Output litoshis is not a natural number'));
    });
  ...
  var expectEqualOutputs = function(a, b) {
    a.litoshis.should.equal(b.litoshis);
  ...
  it('can set a script from a buffer', function() {
    ...
    newOutput.inspect().should.equal('<Output (0 litoshis) <Script: OP_0>>');
  });

  it('has a inspect property', function() {
    output.inspect().should.equal('<Output (0 litoshis) <Script: >>');
  });
  ...
  it('inspect will work with an invalid (null) script', function() {
    ...
    var br = new BufferReader(invalidOutputScript);
    var output = Output.fromBufferReader(br);
    output.inspect().should.equal('<Output (1 litoshis) 4c>');
  });
